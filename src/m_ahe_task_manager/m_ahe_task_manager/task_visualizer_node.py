...
ROBOT_COLOR = {
    'robot_1': (0.0,  0.47, 1.0),
    'robot_2': (0.0,  0.78, 0.31),
    'robot_3': (0.86, 0.20, 0.20),
}

# Quaternion for arrow pointing straight down (90° around Y: default +X → -Z)
_Q_DOWN_X = 0.0
_Q_DOWN_Y = math.sin(math.pi / 4)
_Q_DOWN_Z = 0.0
_Q_DOWN_W = math.cos(math.pi / 4)
...
class TaskVisualizerNode(Node):
    ...
    def _publish_markers(self):
        ...
        # Goal arrows — one downward pin per active robot assignment
        for j, (tid, robot_id) in enumerate(self._assigned.items()):
            task = self._tasks.get(tid)
            if not task or task.get('status') in ('completed', 'failed'):
                continue
            r, g, b = ROBOT_COLOR.get(robot_id, (1.0, 1.0, 0.0))
            ...
            arrow.pose.position.x = task['x']
            arrow.pose.position.y = task['y']
            arrow.pose.position.z = 1.2
            arrow.pose.orientation.x = _Q_DOWN_X
            arrow.pose.orientation.y = _Q_DOWN_Y
            arrow.pose.orientation.z = _Q_DOWN_Z
            arrow.pose.orientation.w = _Q_DOWN_W
            arrow.scale.x = 0.9   # shaft length
            arrow.scale.y = 0.09  # shaft diameter
            arrow.scale.z = 0.14  # head diameter
            arrow.color.r = r
            arrow.color.g = g
            arrow.color.b = b
            arrow.color.a = 0.9
            arrow.lifetime = Duration(seconds=1).to_msg()
