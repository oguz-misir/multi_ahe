...
EXPERIMENT_TIMEOUT_SEC = 360.0   # 6 minutes max per experiment
...
class ExperimentRunnerNode(Node):
    ...
    def _maybe_allocate(self) -> None:
        ...
        if (self._force_realloc
                and isinstance(self._allocator, AHENoEventReplanningAllocator)
    ...
    def _write_summary_csv(self) -> None:
        ...
        failed_tasks = sum(1 for t in self._tasks
                           if not t.completed and not t.active)
        active_remaining = sum(1 for t in self._tasks if t.active and not t.completed)
