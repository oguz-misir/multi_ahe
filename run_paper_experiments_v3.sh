...
#   GV — Video          : G1 yöntemleri × 3 senaryo × seed=01 = 12 video
#   TOPLAM              : 120 veri deneyi + 12 video (~36 + 4 saat)
#
...
# Seed: 01-05  (video fazı yalnızca seed=01)
#
# Kullanım:
#   nohup bash run_paper_experiments_v3.sh \
#       >> results/raw/gazebo_v3/paper_run_v3.log 2>&1 &
#
#   # Yalnızca video fazını tekrar çalıştır:
#   nohup bash run_paper_experiments_v3.sh --video-only \
#       >> results/raw/gazebo_v3/paper_run_v3.log 2>&1 &
# =============================================================================
...
run_video_group() {
    ...
    echo "  $label"
    echo "════════════════════════════════════════════════════════════════"
    # --skip-done 0: DONE olsa bile seed=01 deneyleri video için tekrar koşar
    bash "$REPO/run_experiments_robust.sh" --record-video --skip-done 0 --seeds "1" \
        "$@" --results-dir "$LOG_DIR" >> "$log" 2>&1
    local nvid
    nvid=$(find "$LOG_DIR" -name "video_gazebo.mp4" 2>/dev/null | wc -l)
    echo "  ✓ $label bitti  |  Toplam video: $nvid adet"
}

echo "════════════════════════════════════════════════════════════════"
echo "  AHE-MRTA v3  RA-L Makale Deneyleri"
echo "  120 veri deneyi + 12 video (~36 + 4 saat)"
echo "  Başlangıç: $(date)"
echo "  Sonuç dizini: $LOG_DIR"
if [ "$VIDEO_ONLY" -eq 1 ]; then
    echo "  Mod: YALNIZCA VİDEO FAZI"
fi
echo "════════════════════════════════════════════════════════════════"
...
# ── GV: Video fazı (G1 yöntemleri × 3 senaryo × seed=01 = 12 video) ──────────
run_video_group "GV  Video / G1 yöntemleri × seed=01  (12 video)" "$LOG_DIR/gv_video.log" \
    --robots 3 --tasks 15 \
    --combos "$COMP_COMBOS"
