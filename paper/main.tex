...
expires, and changes hands while the fleet moves. Online multi-robot task
allocation must therefore stay sound as tasks arrive, deadlines tighten, and
robots fail mid-execution; no fixed rule serves every regime. We present
AHE-MRTA, which selects online among five interpretable allocation behaviours
using a two-level rule---deterministic context overrides on failure and
deadline pressure, over a dominance dynamic---and prices assignments with a
...
scenario on all three layers. Under deadline pressure it leads effective
on-time completion ($0.650$ at 5 robots against $0.578$, and $0.764$ at 10
against $0.524$), cutting the strongest baseline's deadline-violation rate by
$17\%$ and $49\%$ while deciding in $0.5$--$3.4$~ms; at the primary scale 20
of 63 cross-method comparisons survive Bonferroni correction, 15 in its
favour. A pre-registered matched
ablation (240 further runs, four arms, 20 common seeds) then separates what
carries that result from what does not: pinning the allocator to a paradigm
the context does not call for costs $9$--$11$ percentage points of effective
on-time completion (large effect, corrected $p<0.005$), whereas removing the
override cascade changes nothing measurable. An unregistered fifth arm sharpens
the bound further---pinned to the one paradigm the selector's fallback returns
almost always, the allocator matches it ($0.638$ against $0.650$, $p=0.43$).
We therefore claim that the portfolio must hold a paradigm fit for the regime
and that the allocator must not be pinned away from it, and explicitly neither
the particular selector we implemented nor its advantage over that fallback.
\end{abstract}
...
We state at the outset what our own measurements support, including where
they contradicted our expectations. A matched closed-loop ablation
(Section~\ref{sec:gazebo-ablation}), pre-registered before it was run, shows
that the \emph{portfolio} is load-bearing---pinning the allocator to a
...
effects---while the
two-level selector's upper layer is not: deleting the override cascade and
falling back to $\arg\max_i D_i$ changes nothing measurable. The two levels are
substitutes, not a mechanism and its ornament. A fifth arm, added after the
registration and reported as exploratory, marks the other edge of the claim:
pinned to spatial-greedy---the paradigm that fallback returns in $99.9\%$ of
the states it decides---the allocator performs like the full selector. We
therefore claim online switching over a fixed portfolio and not the specific
rule that performs it, and we mark clearly where the evidence stops
(Section~\ref{sec:limitations}). The allocation-only ablation separates the
variants too, and the proxy agrees with the fifth arm rather than with the
registered ones: it puts fixed spatial-greedy ahead of the full selector. It
charges nothing for congestion or recovery latency,
which is what the closed-loop experiment adds.
...
\begin{enumerate}
  ...
  they do not: all three rank the methods alike, but only the closed loop
  separates the portfolio (load-bearing) from the selector rule (not).
\end{enumerate}
...
AHE-MRTA leads every scenario---$0.994$, $0.727$ and $0.835$ against $0.978$,
$0.678$ and $0.738$ for the best baseline in each (Table~\ref{tab:fitness},
left). Deterministic navigation therefore does not put the leading methods at
...
separation is entirely in DVR, which AHE-MRTA* holds at $0.350$ where the
strongest baseline (Consensus-DBTA, the best baseline by the same measure)
records $0.422$---a $17\%$ relative reduction. At 10 robots the same
...
are two distinct trade-offs, both reported in Section~\ref{sec:results-gazebo}:
decision latency against the two commit-once baselines, and average task delay
...
other two and one to two orders of magnitude faster than RoSTAM-EA. All four
stay far inside the 50~ms budget, so the ordering is a property of the
allocators rather than a constraint on deployment.
...
\begin{figure}[t]
  ...
  runs have zero re-dispatches, and every run that has any has exactly
  one, spread over distinct seeds---three for AHE-MRTA, six for
...
co-leader in every cell and cuts DVR by $17\%$ (5r) and $49\%$ (10r) against
...
in-window failure recovery---6 of the 60 runs at 3 robots and 3 of 80 at
5 robots; the
...
bounded load-repair step. All three evaluation layers now rank the methods
alike. On the 500-seed navigation-proxy planes AHE-MRTA leads every scenario,
...
$0.641$, $p{=}1.000$); an exploratory fifth arm pinned to the paradigm the
