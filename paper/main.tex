...
selects the behaviour. AHE-MRTA also uses a cached geodesic estimated time of
arrival (ETA) cost oracle and a terminal, bounded load-repair step.
...
In the allocation-only campaign, AHE-MRTA reaches the observed ceiling for
...
mixed-stress edge for AHE-MRTA. AHE-MRTA also increases decision latency and
reduces distance balance under deadline pressure relative to the commit-once
baselines. The four-method Gazebo benchmark uses five seeds per
...
uses the cached geodesic ETA of Eq.~\eqref{eq:f58-eta} as its route metric.
...
Table~\ref{tab:f58-allocation} shows that AHE-MRTA has
