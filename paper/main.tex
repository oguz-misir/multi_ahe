...
\begin{figure}[t]
  ...
  cooperation and suppression matrices $A$ and $S$ are given in
  Table~\ref{tab:as-matrices} and the fixed coefficients in
...
matrix never leave the ecosystem manager. Figure~\ref{fig:comm} shows the resulting
per-event payload: 84~bytes for AHE-MRTA against 134 and 239~bytes for the
commit-once baselines and $1.7{\times}10^{4}$~bytes for RoSTAM-EA, whose
population exchange dominates. The interface is therefore a queue broadcast
whose size is set by the fleet, not by how often the paradigm switches; we
report the payload only and make no claim about its behaviour as a function of
switching frequency, which this experiment does not vary.
