...
baseline), at the cost of higher decision latency, longer routes, and lower
workload balance
...
\begin{figure}[t]
  ...
  illustrative 10-robot / 50-task run with staggered task arrivals, outside
...
$n{=}15$ per cell. There AHE-MRTA takes 13.5--13.7~ms per decision, against
...
5-robot scale are 16--22~ms for AHE-MRTA, 0.13--0.30~ms for BiG-MRTA,
...
not. Consensus methods have nearly perfect workload balance and few messages
because they commit assignments. AHE trades some of this balance and latency
for reactive changes and the reported deadline outcomes: its all-robot Jain
balance is 0.85--0.99 against 0.93--1.00 for the commit-once baselines, and
its routes are longer (99--113~m versus 44--94~m at 3 robots) because
...
commit-once baselines but $1.8$--$3.5{\times}$ below RoSTAM-EA, which grows
to 48--107~ms at 10 robots---and within the real-time budget at both
scales.
...
seed-averaged completion curve across the injection, and the per-run spread of
recovery time, delay, and redispatch. Figure~\ref{fig:timeline} gives the
...
\begin{figure}[t]
  ...
  baselines and below RoSTAM-EA's 48--107~ms. The $n{=}5$ cross-method cells
...
distance (99--113~m versus 44--94~m at 3 robots) and, at the smaller scales,
in makespan under \textit{mixed\_stress} (243~s versus 182--219~s at 3
robots). Redispatch churn is \emph{not} where the cost appears: executed
reassignments stay at or below 0.01 per task, in the best band of the four
methods.
...
\emph{no} configuration statistically separates from full EDPS, and removing
...
fixed paradigm, all leave navigation-proxy fitness within $0.8$~pp of EDPS.
...
(ix) \textbf{Allocation-fitness model.} The navigation-proxy simulator uses a
...
and $49\%$---at the cost of higher decision latency, longer routes, and lower
workload balance.
