...
We evaluate allocation and execution in three separate settings: a
deterministic allocation-only campaign with 100 seeds per cell, a stochastic
navigation proxy with 500 seeds in the primary cells, and closed-loop
...
\label{eq:dominance}
\end{equation}
where $\operatorname{Proj}_{\Delta^4}$ denotes the implementation's
non-negative $\ell_1$ normalization (with a uniform fallback when the sum is
zero), rather than an iterative Euclidean projection. The rule combines interaction-inspired
...
prevent one robot from winning conflicting tasks. It produces highly stable assignments
...
navigation-proxy cells are separate controlled tests. The matched F45--F58 Gazebo study uses paired tests
...
makespan decrease is 18.4~s. The 100-seed-per-cell allocation-only campaign
remains the higher-powered configuration-level evidence.
...
\begin{figure}[t]
  ...
  methods ($p{>}0.3$); AHE-MRTA converts its comparable recovery time
  into the highest completion rate of the reactive methods by escalating
  to the orphan-rescue paradigm within one ecosystem update cycle
  (${\leq}2$~s).}
  \label{fig:recovery}
\end{figure}
...
adds geodesic ETA and bounded load repair. In the 100-seed-per-cell
allocation-only campaign, the compared F45 and F58 configurations reach the
same completion and deadline ceiling.
