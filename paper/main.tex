...
%==============================================================================
\begin{abstract}
Online multi-robot task allocation (MRTA) must preserve good decisions as
tasks arrive, robots fail, deadlines approach, and execution exposes new
constraints. No single fixed allocation policy is best across these regimes: a
rule tuned for one exposes weaknesses in another. We present AHE-MRTA, which
makes the choice of allocation behaviour an online decision through
Ecosystem-Driven Paradigm Selection (EDPS). EDPS is an interpretable,
event-triggered selector: four context signals---task density, robot
availability, deadline pressure, and recent failure rate---drive five hormone
states, each activating one fixed, well-understood behaviour (spatial-greedy,
priority-first, deadline-temporal, commit-once, or orphan-first recovery), with
explicit overrides for acute failure and deadline regimes and a fixed
dominance dynamic otherwise. A cached geodesic-ETA cost oracle and a terminal,
bounded load-repair step complete the method, which decides within a 50~ms
budget.

We evaluate across three complementary layers. In a deterministic
allocation-only campaign (100 seeds per cell), AHE-MRTA reaches the completion
and deadline ceiling in every scale--scenario cell, so this setting does not
separate the leading policies. In a stochastic navigation proxy (500 seeds),
AHE-MRTA and Consensus-DBTA are statistical co-leaders, the only significant
separation being a small mixed-stress edge for AHE-MRTA ($+0.007$,
$p{=}0.042$). In closed-loop Gazebo Harmonic/ROS\,2 Nav2 experiments with 3,
5, and 10 TurtleBot3 robots (300 runs), AHE-MRTA reaches full completion under
robot failure and the highest effective on-time completion under deadline
pressure (about $0.79$ at 5 robots and $0.76$ at 10, versus at most $0.66$ and
$0.52$ for the baselines---roughly $19\%$ and $46\%$ higher than the strongest
baseline), at the cost of higher decision latency and lower distance balance
than the sub-millisecond commit-once baselines. The
four-method benchmark uses five seeds per cell and is reported descriptively:
we make no broad-superiority claim, do not causally isolate EDPS from other
execution-stack differences, keep the allocation-only and closed-loop evidence
separate, and treat the 500-seed navigation-proxy study as the primary
hypothesis test.
\end{abstract}
...
requirements, and operational constraints. During operation, it also depends
on execution state. A robot may be delayed, unavailable, or committed
...
and deadline pressure each reward a different position, a strategy tuned for
one regime is exposed in another. This motivates treating the allocation
behaviour itself as an online choice rather than a fixed design commitment.
...
strong score in a navigation-independent allocator---one measured where every
dispatched goal is reached, so navigation cannot fail---does not by itself
demonstrate closed-loop navigation or recovery performance. Because methods
observe and change the system in different ways, we state the observation
model, reallocation event, communication assumptions, and outcome metrics for
each comparison.
...
\begin{enumerate}
  ...
  \item It separates allocation-only, stochastic navigation-proxy, and
  closed-loop Gazebo evidence, and states the boundary that keeps map-aware
  ETA and recovery effects distinct from a causal claim about EDPS.
\end{enumerate}

The evaluation has three layers that differ in how navigation is modelled. The
\emph{navigation-independent} allocation-only simulation lets every dispatched
goal succeed deterministically, so its scores isolate allocation and
queue-ordering quality from local-planner noise. The stochastic navigation
proxy keeps navigation cost fixed but allows position-dependent goal failure,
adding resilience to the measurement. The closed-loop Gazebo/ROS\,2 stack runs
the full physics and Nav2 pipeline. The separation prevents a navigation or
recovery effect from being reported as allocation evidence. The closed-loop benchmark compares full methods; it does not test
...
and the other base features are dimensionless. The correction $\Psi_{r\tau}^k$ collects the
...
the open task pool, and the previous queues, updates its context and internal
state, activates one paradigm, and publishes robot-specific queues. Adaptation
therefore acts on two coupled layers---\emph{which} behaviour runs, and
\emph{how} the shared cost function is weighted---both driven by the same
context, so a single interpretable state governs the entire decision. The
selector runs no new inter-robot negotiation protocol: the central manager
publishes only each robot's own queue, keeping a low-bandwidth interface.
...
performance and context compatibility \cite{peng2023competition}.
Functionally, it is a fixed, low-dimensional smoothing of paradigm preference:
context and recent outcomes nudge the dominance vector, while the
cooperation--suppression terms damp abrupt swings, giving learning-free
hysteresis. It is not an online learned model and provides no optimality
guarantee; all matrices and coefficients are fixed, and acute failure or
deadline regimes do not depend on it---they are handled by the explicit
overrides of Section~\ref{sec:dispatch}.
...
load-repair step. Neither component changes the EDPS selector or the paradigm
...
The geodesic ETA and bounded repair change execution cost but leave the EDPS
...
0.988--0.992 at 3r, 0.984--0.986 at 5r, and 0.982--0.990 at 10r. Decision
latency is at most 0.201~ms, including the largest 10r/50t cell. During
...
closed-loop Nav2 stack, geodesic repair instead gives the 13--30~ms latency
...
AHE-MRTA* also has the largest reported effective
...
The four-method comparison cannot attribute a Gazebo difference to EDPS
...
\begin{itemize}
  ...
  \item \textbf{No evident advantage when a single regime dominates the whole run and
    ...
    completion (RoSTAM, Cons-DBTA and AHE all at CR\,=\,1.000), so
    AHE-MRTA shows no meaningful completion edge over them while
...
The eight variants span a mean-fitness range of only $[0.469,0.476]$
(spread $0.007$, $\ll$ one standard deviation): \emph{no} configuration
...
(x) \textbf{Missing physical EDPS ablation:} The four-method
...
completion under robot failure and the largest effective on-time completion
...
and $49\%$---at the cost of higher decision latency and lower distance balance.
...
experiment is a matched, multi-map Gazebo ablation of EDPS against fixed
