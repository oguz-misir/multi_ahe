...
%==============================================================================
\begin{abstract}
A robot team is rarely handed all of its work up front: work appears, expires,
and changes hands while the fleet moves. Online multi-robot task allocation
must therefore stay sound as tasks arrive, deadlines tighten, and robots fail
mid-execution, and no fixed rule serves every regime. We present AHE-MRTA, an
event-triggered allocator that chooses online among five interpretable
allocation behaviours. A two-level rule makes the choice: deterministic context
overrides fire on failure and on deadline pressure, and a dominance dynamic
decides when neither fires. Two further mechanisms price the assignment---a
geodesic-ETA oracle and a bounded terminal load repair. We evaluate on three
layers of realism: allocation-only, a stochastic navigation proxy (500 seeds),
and closed-loop Gazebo/ROS\,2 Nav2 with 3, 5, and 10 TurtleBot3 robots
(480 runs, 20 seeds per cell at the primary scale). AHE-MRTA leads every
scenario on all three layers. Under deadline pressure it reaches $0.650$
effective on-time completion at 5 robots and $0.764$ at 10, against $0.578$
and $0.524$ for the strongest baseline. It cuts that baseline's
deadline-violation rate by $17\%$ and $49\%$, and decides in $0.8$--$3.4$~ms at
those two scales. At the primary scale, 20 of 63 cross-method comparisons
survive Bonferroni correction: 15 in its favour and five against.
A pre-registered matched ablation (240 further runs, four arms, 20 common
seeds) then separates what carries that result from what does not. Pinning the
allocator to a paradigm the context does not call for costs $9$--$11$
percentage points of effective on-time completion (large effect, corrected
$p<0.005$); removing the override cascade changes nothing measurable. An
unregistered fifth arm narrows the claim further: pinned to the one paradigm
the selector's fallback returns almost always, the allocator merely matches it
($0.638$ against $0.650$, $p=0.43$). We therefore claim that the portfolio must
hold a paradigm fit for the regime, and that the allocator must not be pinned
away from it. We claim neither the particular selector we implemented nor any
advantage of it over that fallback.
\end{abstract}
...
that helps in one regime tends to hurt in another. Three tensions recur.
The first is solution quality against reaction cost. Centralized formulations
jointly optimize assignment and routing when global state is available, folding
timing, workload, distance, energy, and deadline terms into one program
\cite{lei2023convex}. Re-solving that coupled problem at every event is
expensive, and it can overturn a plan execution has already begun to act on.
The second is global optimality against coordination cost. Distributed and
market-based schemes replace the global solve with local decisions and explicit
messaging: group-based auctions serve delivery requests that arrive over time
with time windows \cite{bai2023group}, and consensus-based allocation gives up
some optimality for local coordination, speed, and lower energy use
\cite{mahato2023consensus}. The locality that bounds their communication,
however, also limits how far they exploit global structure.
The third is reactivity against stability. An aggressively re-optimizing policy
recovers quickly when a robot fails or a deadline slips, but it preempts
running tasks and breaks feasible routes; a commit-once policy is cheap and
stable, but it strands the tasks of a failed robot.
Other online methods take fixed positions on these axes: game-theoretic
clustering couples task grouping with allocation
\cite{garciamartin2023clustering}, self-adaptive methods react to shifts in
problem structure \cite{rostam2024selfadaptive}, and decentralized
energy-aware policies weight power draw \cite{bagchi2024energyaware}. Each
commits to its position in advance. Arrival bursts, robot failures, and
deadline pressure each reward a different position, which is why we treat the
allocation behaviour itself as an online choice rather than a fixed design
commitment.

This paper considers online single-task-robot allocation under task arrivals,
...
We state at the outset what our own measurements support, including where they
contradicted our expectations. A matched closed-loop ablation
(Section~\ref{sec:gazebo-ablation}) was pre-registered before it was run. It
shows that the \emph{portfolio} is load-bearing: pinning the allocator to a
paradigm the context does not call for costs $9$--$11$ percentage points of
effective on-time completion under deadline pressure, with large corrected
effects. It also shows that the selector's upper layer is not load-bearing.
Deleting the override cascade and falling back to $\arg\max_i D_i$ changes
nothing measurable, so the two levels are substitutes rather than a mechanism
and its ornament. A fifth arm, added after registration and reported as
exploratory, marks the other edge of the claim. Pinned to spatial-greedy---the
paradigm that fallback returns in $99.9\%$ of the states it decides---the
allocator performs like the full selector. We therefore claim online switching
over a fixed portfolio, not the specific rule that performs it, and we mark
clearly where the evidence stops (Section~\ref{sec:limitations}).
The navigation-proxy ablation separates the variants too, and it agrees with
the fifth arm rather than with the registered ones: it puts fixed
spatial-greedy ahead of the full selector. That plane charges nothing for
congestion or recovery latency, which is exactly what the closed-loop
experiment adds.
...
\begin{enumerate}
  ...
  closed-loop Gazebo evidence separately, and shows where they agree and where
  they do not: all three put AHE-MRTA first in every scenario, but the ordering
  \emph{among} the baselines does not carry across planes---commit-once
  BiG-MRTA is a consistent runner-up on the proxy and last in two of three
  Gazebo scenarios---and only the closed loop separates the portfolio
  (load-bearing) from the selector rule (not).
\end{enumerate}
...
recovery performance. The closed-loop benchmark compares full methods; the causal effect of the
selector is isolated separately, by the matched ablation of
Section~\ref{sec:gazebo-ablation}.
...
urgent work, and it behaves as one as long as the bracket is positive. In the
evaluated configuration the bracket is negative: the deadline-capability bonus
inside $\Psi_{r\tau}^k$ outweighs the weighted features. Multiplying a negative
bracket by $\rho$ inverts the intent, so $\rho$ charges a priority-3 task
\emph{more} than a priority-1 task in the same state. We report the implemented
form rather than the intended one, and quantify the effect in
...
$\ell_k=t_k^{\rm publish}-t_k\leq50$~ms. It is met per event, not only on
average: across the 1{,}628 allocation decisions logged in the 60 AHE-MRTA
runs at the primary scale, the slowest single decision is $4.35$~ms and none
exceeds the budget. Baseline decision latencies are
...
$\mathcal R^{\rm alive}_k$ denote robots that are neither failed nor
navigation-stuck, and $\mathcal R^{\rm f}_k$ its complement. Queue occupancy
does not enter $c_{2,k}$: the context is assembled by the ecosystem manager
from published robot-status summaries, which carry health and navigation state
but not queue length. $\mathcal R^{\rm alive}_k$ is therefore defined by a
weaker condition than the allocator-side $\mathcal R^{\rm av}_k$ of
Section~\ref{sec:problem}, and is a superset of it. The
components are
\begin{align}
...
made. Replaying the selector over the $15{,}058$ logged states of the benchmark
campaign, the failure override decides $49.2\%$ of events and the deadline
override a further $25.3\%$, leaving $25.5\%$ to the fallback
...
the counter is reset to $K_{\rm dwell}=4$. We use a distinct symbol here
because $\rho$ already denotes the priority factor of Eq.~\eqref{eq:cost}.
H\_RECOV bypasses this hold, so a detected
...
three of the $15{,}058$ replayed states. We report it because the system runs
...
and recovery mode is inactive, the urgency channel is multiplied by $1.5$ and
clipped at $1$. These weights are the context-dependent part of the common cost
in Section~\ref{sec:problem}. The evaluated execution stack has no battery
model, so the battery \emph{feature} is identically zero in every reported run
and the channel contributes nothing to any cost, even though its weight
$w_b$ is not zero (Section~\ref{sec:method-config}). Channel and weight are
kept in the cost vector so that the same weight map transfers unchanged to
...
estimated arrival; three classes are exempt from this margin rule---orphans
whose previous owner is no longer healthy, rescue tasks, and tasks already
inside the urgent horizon---so the gate never delays work that is failing or
about to expire.
...
per-queued-task navigation allowance. The speed $v_r=0.22$~m/s is the
\emph{effective} traversal rate, not the platform maximum
$v_{\max}=0.26$~m/s of Section~\ref{sec:hardware}: it absorbs planning and
acceleration overhead, and it is the value the implementation divides by. This
deliberately inexpensive
...
\begin{figure}[t]
  ...
  \caption{One allocation event, end to end. \textbf{Row A} decides
  \emph{which} behaviour handles the event. Context sensing (1) compresses the
  state into $c(t)$; the deterministic override cascade (2) is checked in the
  order shown and settles most events on its own; only when neither threshold
  fires does the dominance fallback (3) choose; dwell hysteresis (4) then
  publishes the active paradigm $p^\ast$. The two tiers are not equal partners:
  replayed over the $15{,}058$ logged ecosystem states of the 120 AHE-MRTA
  benchmark runs, the cascade decides $74.5\%$ of events, and replacing the
  fallback with a constant changes 3 of those selections
  (Section~\ref{sec:limitations})---which is why the fallback is drawn muted.
  \textbf{Row B} turns $p^\ast$ into queues: the portfolio (5) fixes task order,
  the cached geodesic-ETA oracle (6) prices and masks the pairs, the assignment
  (7) is taken under single-owner and in-flight locks and closed by the terminal
  bounded load repair, and (8) publishes per-robot queues. Execution feedback
  re-enters at (1). Fixed coefficients are listed in
  Section~\ref{sec:method-config}.}
  \label{fig:mechanism}
\end{figure}
...
$\eta=\lambda=0.12$, $\delta=0.20$, $T_w=0.3$, and $K_{\rm dwell}=4$.
...
\label{sec:scales}
We evaluate three Gazebo scales (Table~\ref{tab:scales}). Each scale--density
cell contains 60 runs ($4$ methods $\times$ $3$ scenarios $\times$ $5$ seeds),
except the primary 5-robot cell, which carries 20 seeds per
method--scenario combination for 240 runs.
...
differ from AHE in allocation logic as well as paradigm choice. Two separate
ablations address the selector question directly---one on the proxy plane
(Section~\ref{sec:ablation}) and a pre-registered matched one in the closed
loop (Section~\ref{sec:gazebo-ablation}).
...
not a universal ranking of the cited algorithms. Attributing a closed-loop
difference to the selector itself requires a controlled Gazebo ablation against
fixed paradigms; that experiment was pre-registered and run, and
Section~\ref{sec:gazebo-ablation} reports what it does and does not support.
...
tests per scenario). At the primary 5-robot scale, each cell has $n{=}20$ runs
...
$\delta$, an effect-size estimate suited to small cells. The 10-robot
Gazebo cells, at $n{=}5$, are descriptive. We make no cross-method
...
\input{table/allocation_only.tex}

Table~\ref{tab:allocation} reports AHE-MRTA across the three scales (100 seeds
per cell). Completion is exact---CR${=}1.000$ in all nine scale--scenario
cells---but fitness is not, because a task that finishes after its deadline
earns no credit even when nothing is left unserved: priority-weighted on-time
completion runs from $0.647$ to $1.000$. Deadline violation is what separates
the cells, and it tracks the scenario rather than the scale: $0.000$--$0.036$
under robot failure, $0.116$--$0.232$ under deadline pressure, and
$0.215$--$0.353$ under mixed stress. The active-robot Jain index is
$0.983$--$0.994$ at every scale, and decision latency is at most $0.215$~ms,
in the 10r/50t mixed-stress cell. During
...
Two things follow. Fitness rises with fleet size in both stressed scenarios
($0.774\to0.888$ under deadline pressure and $0.647\to0.782$ under mixed
stress, from 3 to 10 robots), which is what more robots buy against unchanged
per-task budgets. And the plane separates the policies: at the primary scale
AHE-MRTA leads every scenario---$0.994$, $0.727$ and $0.840$ against $0.980$,
$0.668$ and $0.733$ for the best baseline in each, BiG-MRTA in all three
(Table~\ref{tab:fitness}, left). Deterministic navigation therefore does not put the leading methods at
a common ceiling; it removes navigation failure as a confound and leaves
allocation and queue order to do the separating.
...
per scenario, AHE-MRTA leads every scenario on both planes
(Table~\ref{tab:fitness}), and eight of the nine paired comparisons against a
baseline are significant, most of them overwhelmingly
($p$ from $10^{-9}$ to $10^{-76}$). The exception is a tie with BiG-MRTA under
robot failure ($+0.004$, $p{=}0.14$).

\input{table/fitness.tex}

At the primary scale (25 tasks and 5 robots; Table~\ref{tab:fitness}, left),
no method reaches the ceiling: with the deadline budgets the scenarios
actually specify, on-time completion separates the four policies even when
navigation cannot fail. AHE-MRTA leads at $0.994$, $0.727$, and $0.840$ for
robot failure, mixed stress, and deadline pressure, against $0.980$, $0.668$,
and $0.733$ for the best baseline in each. When navigation becomes stochastic
(Table~\ref{tab:fitness}, right) every method falls further, and the ordering
is preserved: $0.384$, $0.263$, and $0.349$ against $0.380$, $0.229$, and
$0.284$. The margin is smallest under robot failure, where BiG-MRTA's
commit-once discipline is well matched to the regime and the difference is not
significant; it is largest under deadline pressure, which is the regime the
overrides and the geodesic ETA both target.

An earlier version of this evaluation reported these two planes as saturated
and unable to separate the leading policies. That was an artefact of the
simulator's scenario parameters having drifted from the ones documented in
Section~\ref{sec:scenarios}: deadline budgets were drawn from
$\mathcal U[200,400]$~s where the design specifies
$\mathcal U[36,120]$~s under deadline pressure, and the halved budget and
staggered arrivals of mixed stress were absent. Under budgets that generous
every policy completes almost everything on time, so nothing could separate.
Both planes now read their scenario definitions from the same module the
Gazebo runner uses, and a parity test fails if the two ever diverge again.
The proxy still does not isolate which mechanism produces the margin, and it
does not predict a closed-loop Nav2 result: it charges no cost for congestion,
replanning latency, or recovery time, all of which the physical stack does.
Section~\ref{sec:results-gazebo} reports the Gazebo estimates separately, and
Section~\ref{sec:gazebo-ablation} is where the mechanism question is settled.
Figure~\ref{fig:fitness} summarizes the proxy results.
...
\begin{figure}[t]
  ...
  1.5 interquartile ranges; points are outliers. AHE-MRTA leads every scenario;
  the paired-Wilcoxon comparisons against each baseline are significant in
  eight of nine cases, the exception being a tie with BiG-MRTA under robot
  failure ($p{=}0.14$).}
  \label{fig:fitness}
\end{figure}
...
AHE-MRTA is highest on proxy fitness at every scale
(Table~\ref{tab:scalability}), by $3.3$, $3.6$, and $2.7$~pp over the best
baseline at $N=3$, $5$, and $10$; the runner-up is BiG-MRTA at all three.
The margin does not grow with fleet size, so we read the result as a
consistent advantage rather than a scaling one. Proxy decision latency remains
about $0.54$~ms at 10 robots. This is nearly four orders of magnitude below the 5~s replanning
period and about 13 times lower than RoSTAM-EA (7.0~ms at $N=10$). In the
...
The 5-robot Gazebo study contains 240 runs and the 10-robot study 60. Their
per-scenario results appear with the 3-robot benchmark in
Section~\ref{sec:results-gazebo}. They are not a direct confirmation of the
proxy because they use a different noise model and, at 10 robots, far fewer
seeds.
...
\begin{figure}[t]
  ...
  latency. AHE-MRTA is highest on fitness at every scale, ahead of the
  runner-up (BiG-MRTA) by $3.3$, $3.6$, and $2.7$~pp at $N=3$, $5$, and $10$;
  the margin does not grow with fleet size.}
...
method--scenario combination give 180 runs. The 5r study adds 240 (20 seeds
per method--scenario combination) and the 10r study 60, for 480 runs in
total. We use Bonferroni-corrected Mann--Whitney U
...
\begin{figure*}[t]
  ...
  seed 1) for AHE-MRTA, Consensus-DBTA, BiG-MRTA, and RoSTAM-EA, in reading
  order from top left. These are the same four runs that enter the 5r/25t
  \textit{mixed\_stress} cells of Table~\ref{tab:main_comparison}. Squares are
  spawn positions, gold stars are sampled waypoints, and coloured curves are
  paths reconstructed from ground-truth poses; the timeseries logs each pose in
  its robot's odom frame, so every trace is shifted by that robot's spawn
  point. A square without a curve is a robot that never left its spawn, as
  Consensus-DBTA's robot~1 does here. This single-run figure is qualitative; it
  does not establish an aggregate method ranking.}
...
$\mathrm{CR}{\times}(1{-}\mathrm{DVR})$, is $0.650$ for AHE-MRTA* at 5 robots
against $0.578$ for the strongest baseline, and $0.764$ against $0.524$ at 10
robots. The margin comes from deadline compliance, not from throughput. Three
of the four methods complete essentially every task at 5 robots, so the
separation lies entirely in DVR: AHE-MRTA* holds it at $0.350$, while the
strongest baseline by the same measure, Consensus-DBTA, records $0.422$. That
is a $17\%$ relative reduction. At 10 robots the same comparison is $0.220$
against $0.428$, a $49\%$ reduction, and the baselines reach their completion
rates only by tolerating those violations. This is the pattern the method was
built for: the deadline override and the geodesic ETA both act on time slack,
and this is the regime where slack is scarce.

At the primary scale these are corrected results, not point estimates: with
$n{=}20$ per cell, 20 of 63 cross-method comparisons survive Bonferroni
correction, 15 of them in AHE-MRTA's favour. The five that go the other way
are two distinct trade-offs, both itemised later in this section: decision
latency against commit-once BiG-MRTA in all three scenarios and against
Consensus-DBTA under mixed stress, and average task delay
against Consensus-DBTA under robot failure. The 10-robot cells still carry
$n{=}5$ and remain descriptive.
...
metrics. At this scale AHE-MRTA completes every task in every scenario
(CR\,=\,1.000); at 10 robots it does not, reaching $0.976$--$0.988$.
BiG-MRTA has lower point estimates under
\textit{deadline\_pressure} (0.664) and \textit{mixed\_stress} (0.656).

The four-method comparison cannot attribute a Gazebo difference to the selector
because methods differ in several components. The matched fixed-paradigm and
no-override ablation of Section~\ref{sec:gazebo-ablation} is what separates
those factors, and it assigns the pattern to the portfolio rather than to the
override cascade.

\paragraph{Costs of AHE.}
Reactive behavior can increase execution costs, and at the primary scale two
of them are now measurable rather than merely suggestive. Under
\textit{robot\_failure}, AHE's average delay is 99.7~s against
Consensus-DBTA's 86.0~s---a corrected-significant gap
($p_{\mathrm{adj}}{=}0.002$, $\delta{=}0.72$) and one of the five comparisons
that go against AHE-MRTA. RoSTAM-EA (92.6~s) and commit-once BiG-MRTA
(104.4~s) sit either side of it and neither separates. Its 83.7~s recovery
time is lower than Consensus-DBTA's 117.9~s and higher than RoSTAM-EA's
74.5~s and BiG-MRTA's 73.1~s, but recovery time separates from no baseline
(Figure~\ref{fig:recovery}); the cell is confounded by Nav2 motion in any
case. Under \textit{mixed\_stress} and \textit{deadline\_pressure}, BiG-MRTA
has the largest delay (311--327~s)
...
in the best band: at most 0.014 redispatches per task and zero execution
...
method (e.g., 1.02 for AHE and 5.29 for BiG-MRTA in this cell), so the two
must not be compared.
Decision latency is the second measurable cost, and the direction depends on
the comparison. The geodesic ETA oracle depends only
...
decides in 0.81--1.33~ms at 5 robots and 1.9--3.4~ms at 10. That is
corrected-significantly slower than commit-once BiG-MRTA (0.14--0.24~ms at 5
robots) in all three scenarios, and slower than Consensus-DBTA under
\textit{mixed\_stress}, while remaining faster than Consensus-DBTA under the
other two and one to two orders of magnitude faster than RoSTAM-EA. Three of
the four stay far inside the 50~ms budget, so among them the ordering is a
property of the allocators rather than a constraint on deployment; RoSTAM-EA
does not (49--95~ms at this scale), and for it the ordering is a deployment
constraint as well.
...
5-robot scale are 0.81--1.33~ms for AHE-MRTA, 0.14--0.24~ms for BiG-MRTA,
0.45--2.43~ms for Consensus-DBTA, and 49--95~ms for RoSTAM-EA. AHE-MRTA
therefore stays within the 50~ms budget at every scale; RoSTAM-EA does
not.\footnote{At the primary 5-robot scale the campaign runs were themselves
executed with the geodesic oracle built at start-up, so the reported AHE-MRTA
latency is the campaign's own. The 3-robot and 10-robot AHE-MRTA latency
figures instead come from a 60-run re-measurement that matches those cells of
the original campaign one for one (four scale--density cells $\times$ three
scenarios $\times$ five seeds); every other AHE-MRTA quantity and all baseline
quantities come from the campaign itself. The change moves map-derived set-up out of the timed
...
pre-warmup campaign's latency (15.4 versus 15.9~ms) while every non-latency
difference stayed inside ordinary Gazebo run-to-run variation.} Consensus methods have
...
\paragraph{Effect sizes.} Table~\ref{tab:effectsize} reports Cliff's $\delta$
alongside the corrected tests, so that the size of a difference can be read
independently of whether it clears the threshold. Its sign is normalized
...
$+0.70$ under \textit{robot\_failure}. Deadline-compliance effects are
uniformly large under \textit{robot\_failure} ($+0.83$ to $+0.86$ against all
three baselines, all corrected-significant); under
\textit{deadline\_pressure} only the RoSTAM-EA comparison is both large and
significant ($+0.81$), with BiG-MRTA at $+0.34$ and Consensus-DBTA at $+0.39$.
The largest unfavorable effect is recovery time versus commit-once BiG-MRTA
under \textit{mixed\_stress} ($-0.50$), which does not survive correction;
recovery against reactive Consensus-DBTA runs the other way ($+0.45$ under
\textit{robot\_failure}). On corrected redispatch churn no comparison is
larger than $|\delta|{=}0.30$ in either direction. Read with the tests, these
sizes say the deadline-compliance advantage is the substantial one and the
execution-cost differences are mostly small.

\input{table/latex_effectsize_table.tex}

\paragraph{Statistical significance.} At the primary 5-robot scale
($n{=}20$ per cell), 20 of 63 cross-method comparisons survive Bonferroni
correction, 15 in AHE-MRTA's favour and 5 against it; 28 of 63 reach raw
$p<0.05$. The 15 favourable results are completion rate and deadline
violation rate against the baselines that lose on each, plus decision latency
against RoSTAM-EA and Consensus-DBTA; the 5 unfavourable ones are decision
latency against commit-once BiG-MRTA in all three scenarios, decision latency
against Consensus-DBTA under \textit{mixed\_stress}, and average task delay
against Consensus-DBTA under \textit{robot\_failure}. At 3 robots
($n{=}15$ per cell), 14 of 63 comparisons survive correction, 8 of them in
AHE-MRTA's favour. At 10 robots the five seeds per cell cannot support
correction at all---the smallest attainable two-sided Mann--Whitney value is
about 0.008, above the corrected threshold of about 0.0024---so none survive
and those cells stay descriptive. The 500-seed navigation-proxy study
(Section~\ref{sec:results-sim}) remains the highest-powered cross-method test;
the Gazebo benchmark now carries corrected inference of its own at the primary
scale.
...
\textit{robot\_failure} $0.986$ (DVR\,=\,0.014) vs
${\leq}0.890$, \textit{deadline\_pressure} $0.650$
(DVR\,=\,0.350) vs ${\leq}0.578$, and \textit{mixed\_stress}
$0.616$ vs ${\leq}0.542$; BiG-MRTA abandons tasks under load
(CR\,=\,0.656/0.664). Decision latency is 0.81--1.33~ms at 5 robots and
1.9--3.4~ms at 10---within the spread of the reactive baselines and
...
\begin{figure}[t]
  ...
  pooled; $n{=}20$ seeds per cell; mean $\pm$ standard deviation). Panels show
  completion rate, effective on-time completion
  $\mathrm{CR}{\times}(1{-}\mathrm{DVR})$, average delay, workload balance,
  deadline violation rate, and log decision latency. Pooling the two scenarios
  is for display only; the corrected tests are run within each scenario and
...
\begin{figure}[t]
  ...
  0.6~ms for commit-once BiG-MRTA, 1.8~ms for Consensus-DBTA, and 69.1~ms for
  RoSTAM-EA.
...
\begin{figure}[t]
  ...
  the primary 5-robot / 25-task scale ($n{=}20$ seeds): recovery time and
  ...
  keeps the rare, discrete nature of the metric visible: 66 of the 80
  runs have zero re-dispatches, and each of the remaining 14 has exactly
  one, spread over distinct seeds---three for AHE-MRTA, two for BiG-MRTA,
  three for Consensus-DBTA, and six for
  RoSTAM-EA (execution preemptions are zero for every method and are
  omitted). No recovery-time comparison survives correction; the closest
  is against Consensus-DBTA, where AHE-MRTA's 83.7~s beats 117.9~s at raw
  $p{=}0.017$ but corrected $p{=}0.35$. Raw completion rate is omitted
  because it does not discriminate here---AHE-MRTA, RoSTAM-EA and
  Consensus-DBTA are all at $1.000$---whereas on effective on-time
  completion AHE-MRTA leads at $0.986$ against $0.884$--$0.890$, i.e.\ it
  converts a comparable recovery time into fewer late completions.}
  \label{fig:recovery}
\end{figure}
...
co-leader in every cell, and it cuts DVR by $17\%$ (5r) and $49\%$ (10r)
against the strongest baseline. It pays for the extra replanning elsewhere---in
total travel distance (99--113~m versus 44--104~m at 3 robots) and, at the
smaller scales, in makespan under \textit{mixed\_stress} (243~s versus
182--219~s for the two other reactive baselines at 3 robots; commit-once
BiG-MRTA runs to the 900~s horizon in this cell).
Redispatch churn is \emph{not} where the cost appears
at the primary scale: at 5 robots executed reassignments stay at or below
0.014 per task, the best band of the four methods. At 10 robots they rise to
...
They are not causal estimates of paradigm switching: the four-method
comparison varies several components at once. The matched ablation of
Section~\ref{sec:gazebo-ablation} is what isolates the selector.
...
\begin{itemize}
  \item \textbf{Deadline-compliance pattern.} On \textit{deadline\_pressure}, the consensus and
    ...
    $1.2$--$2.3\times$ its deadline-violation rate (5r: AHE
    DVR\,=\,0.350 vs Cons-DBTA's 0.422 and RoSTAM-EA's 0.498; 10r: AHE
    0.220 vs 0.428 and 0.504). AHE-MRTA has the largest
    effective on-time completion at both scales
    ($0.650$ at 5r, $0.764$ at 10r). The four-method
  ...
  \item \textbf{Failure-recovery pattern.} Under
    ...
    (CR\,=\,1.000, DVR\,=\,0.014, effective on-time $0.986$
    vs ${\leq}0.890$) with tied-highest completion at 10
...
\subsection{Why the navigation-proxy margins stay narrow}
\label{sec:deadline-tie}
AHE-MRTA leads every scenario on the stochastic navigation-proxy fitness
(priority-weighted on-time completion), but by margins of $0.004$--$0.065$
rather than by a wide separation, and all four methods score far below the
perfect-navigation plane (Table~\ref{tab:fitness}). Two mechanisms explain
both facts. First, the ceiling on this plane is set by \emph{stochastic,
position-dependent navigation failure}, not by assignment quality: the same
policies score $0.642$--$0.994$ when navigation cannot fail and
$0.217$--$0.384$ when it can, so most of the loss is failure and
re-attempt, which no allocator can price away. The binding constraint is
therefore resilience to navigation failure---a capability AHE-MRTA and
Consensus-DBTA both implement by re-dispatching failed and orphaned
tasks---which compresses the field. It compresses it most under robot
failure, where commit-once BiG-MRTA is also well matched to the regime and
finishes within $0.004$ of AHE-MRTA, the one comparison on this plane that
does not separate; the margins widen under mixed stress and deadline
pressure ($+0.034$ and $+0.065$ over the best baseline), where abandoning or
mis-ordering a task is not free.
...
a deadline-feasibility-aware execution ordering could widen the proxy
margin: in isolation it lifts deadline fitness by ${\approx}1.3$~pp,
...
whose leading fitness and physical-stack advantages
...
The variants span $0.101$ in mean fitness, and two readings stand out. First,
removing the context overrides is nearly free on this plane
($0.323$ against $0.329$), as is removing the recovery boost ($0.328$).
Second, one fixed paradigm---spatial-greedy---is \emph{ahead} of the full
selector ($0.348$), while the other four fixed paradigms are behind it by
$0.02$--$0.08$. The proxy therefore suggests both that the override layer is
inert and that the selector may be beaten by the single behaviour its fallback
returns most often.

Neither reading transfers automatically: this plane charges nothing for
congestion, replanning latency, or recovery time, which is where a switch
would pay off. The matched closed-loop ablation below tests both. It confirms
the first---the overrides are inert there too---and contradicts the second for
the two fixed paradigms it registered in advance, which are decisively worse
in the closed loop. Fixed spatial-greedy, which this table promotes, was not
part of the registered design; Section~\ref{sec:gazebo-ablation} reports it
separately as an exploratory arm.

\paragraph{What the geodesic ETA contributes.}
Every arm above prices assignments with the geodesic-ETA oracle, so none of
them says anything about the oracle itself. We isolate it with a second paired
A/B on the same plane. The allocator's \emph{cost} oracle is switched between
geodesic ETA and the Euclidean reference over 500 common seeds, while the
\emph{execution} oracle stays geodesic in both arms, so the two are scored
against identical ground truth. The three baselines come out bit-identical
across the arms, as they must: no baseline reads the flag. The geodesic arm
also reproduces the published campaign of Table~\ref{tab:fitness} to nine
decimals, so the comparison is against the configuration the paper actually
reports. The oracle helps in every scenario: $+2.13$~pp under robot failure,
$+1.83$~pp under mixed stress, and $+4.10$~pp under deadline pressure, each
with $p<10^{-4}$ over 500 pairs. The effect sizes are modest. Cliff's $\delta$
is negligible in the first two scenarios ($0.131$ and $0.128$) and small in the
third ($0.275$). The ordering is the one the mechanism predicts---an accurate
arrival estimate matters most where time slack is scarcest---so we report the
oracle as a consistent but small gain, not as the main carrier of the result.

\subsection{Matched closed-loop ablation}
\label{sec:gazebo-ablation}
The proxy result above prices no congestion, replanning latency, or recovery
time, so it cannot say whether a switch pays off where those costs are real.
We therefore repeated the ablation
in the closed loop, holding every non-selector component fixed: four arms
---\emph{full}, \emph{no-override} ($\arg\max_i D_i$ only), and the two fixed
paradigms the selector actually reaches---over three scenarios and 20 common
seeds at 5 robots / 25 tasks, 240 runs. Hypotheses, arms, endpoints and the
statistical plan were registered before the campaign started and are archived
with the artifact; the analysis code was written and committed while one arm
was still empty. No seed was lost, so all 20 pair in every arm.

\input{table/gazebo_ablation.tex}

Table~\ref{tab:gazebo-ablation} splits the question in two, and the two halves
answer differently.

\textbf{Being able to switch matters.} Pinning the allocator to either fixed
paradigm costs $9$--$11$ percentage points of effective on-time completion
under deadline pressure ($0.650$ against $0.556$ and $0.540$), with large
effect sizes ($\delta=0.57$ and $0.62$) that survive Bonferroni correction
($p<0.005$). The same ordering appears under robot failure, where the two
switching arms reach $0.99$ and the two fixed arms $0.85$. The proxy ranks
these two fixed paradigms below the full selector as well, on the same
scenario and endpoint ($0.348$ against $0.296$ and $0.259$;
Table~\ref{tab:ablation}), so the two planes agree in direction. They differ
in what they support: the closed-loop penalty is larger for fixed-EDF
($9.4$~pp against $5.2$), and only the closed-loop comparison is paired on
common seeds and corrected, which is what turns the ordering into a result.

\textbf{Which mechanism does the switching does not matter.} Removing the
context-override cascade entirely changes nothing measurable: $0.641$ against
$0.650$, $p=1.000$ after correction, $\delta=0.06$ (negligible). Under robot
failure the no-override arm is if anything slightly ahead ($0.998$ versus
$0.986$, and 63~s versus 84~s to recover, neither surviving correction). The
overrides and the dominance dynamic are therefore substitutes rather than a
load-bearing layer and its ornament: either route reaches an adequate paradigm,
and the cost of having neither is large.

\textbf{The paradigm the fallback actually returns.} The two fixed arms above
were registered because they are the paradigms the overrides target. They are
not the paradigm the allocator spends most of its time in: the dominance tier
returns spatial-greedy in $99.9\%$ of the states it decides
(Section~\ref{sec:limitations}), and the corrected proxy ablation puts fixed
spatial-greedy \emph{ahead} of the full selector
(Table~\ref{tab:ablation}). Leaving the strongest candidate untested would
rest the portfolio claim on two weak alternatives, so we ran it as a fifth
arm, matched to the same 20 seeds but outside the registered design and
therefore outside the primary Bonferroni family. On the primary endpoint it
does not separate from the full selector: $0.638$ against $0.650$, paired
$p=0.43$, $\delta=+0.09$ (negligible). Under robot failure it is ahead
($0.998$ against $0.986$, raw $p=0.014$, $\delta=-0.30$) and under mixed
stress behind ($0.584$ against $0.616$, raw $p=0.038$); neither would survive
the correction the registered arms passed.

This bounds the portfolio claim more tightly than the registered arms alone
would. What the closed loop demonstrates is that pinning the allocator to a
paradigm the context does not call for is expensive---fixed-EDF and
fixed-orphan lose $9$--$11$~pp---not that the selector beats every fixed
policy. The fixed policy that matches the allocator's own dominant behaviour
performs like the full selector, as does dropping the override cascade:
$0.650$, $0.641$ and $0.638$ are three descriptions of the same performance.
We therefore claim that the portfolio must contain a paradigm suited to the
regime and that the allocator must not be pinned away from it; we do not claim
that the selector earns its complexity over spatial-greedy in these three
scenarios. Separating those two claims needs regimes where spatial-greedy is
the wrong answer and the selector has to leave it---which the mixed-stress gap
hints at and no experiment here establishes.

Two limits of this experiment are worth stating with the result. The secondary
endpoint we registered for \textit{robot\_failure}, completion rate, turned out
to be at ceiling in all four arms ($\geq0.998$) and so could not discriminate;
effective on-time completion does discriminate there, but we had registered it
for the primary scenario only, so it is reported above as descriptive rather
than promoted after the fact. And the campaign uses one arena: it closes the
matched-ablation gap, not the single-map gap.
...
(i) \textbf{Statistical power varies by scale:} The 5-robot primary scale now
carries $n{=}20$ per cell and supports corrected inference: 20 of 63
cross-method comparisons survive Bonferroni correction, against 0 of 63 when
the same cells carried $n{=}5$. Every method's point estimates fell when the
seed budget was raised---by $0.08$--$0.16$ in effective on-time completion,
consistently across all four---so the earlier five-seed estimates were
optimistic, though the ranking they implied held. The 3-robot cells carry
$n{=}15$ and the 10-robot cells still carry $n{=}5$; the latter remain
descriptive, and a larger 10-robot budget would extend inference to that
scale. The 500-seed proxy planes support their own high-power comparisons.
...
over the $15{,}058$ ecosystem states logged across the 120 AHE-MRTA benchmark
runs quantifies the split: the failure override decides $49.2\%$ of states,
the deadline override $25.3\%$, and the dominance tier the remaining
$25.5\%$---where it returns spatial-greedy in $99.9\%$ of cases. Replacing
that tier with a constant would change the selected paradigm in 3 of
$15{,}058$ states ($0.02\%$); commit-once is never activated in any evaluated
scenario and priority-first fires in a single state. We therefore frame
...
in Table~\ref{tab:ablation} (Sec.~\ref{sec:ablation}) is consistent on the
override layer---removing the overrides costs $0.6$~pp of navigation-proxy
fitness and removing the recovery boost $0.1$~pp---but not on the portfolio:
the fixed paradigms range from $1.9$~pp \emph{above} the selector
(spatial-greedy) to $8.2$~pp below it (commit-once). The closed-loop ablation
of item~(iii) is what adjudicates between those readings.
(iii) \textbf{What the closed-loop ablation does and does not settle:} The
matched experiment of Section~\ref{sec:gazebo-ablation} closes the causal gap
this paper previously listed as open---it holds every non-selector component
fixed over 20 common seeds---but it settles the portfolio question, not the
mechanism question. Switching beats being pinned to a fixed paradigm by a
large, corrected margin; the specific selector we propose is
\emph{indistinguishable} from the plainer $\arg\max_i D_i$ rule it sits on top
of. We therefore claim online switching over a fixed portfolio, and explicitly
do not claim that the override cascade is the reason it works. Establishing
which switching rules suffice, and whether any is better than the rest, needs a
sweep over rules rather than the two points we measured. The experiment also
uses a single arena, so it does not close the generalisation gap of item~(vi).
...
the simulation host, not an algorithmic one: in the navigation-independent
setting the allocator holds CR at $1.000$ and decides in at most $0.215$~ms at
10 robots / 50 tasks, and its fitness \emph{rises} with fleet size rather than
degrading (Table~\ref{tab:allocation}). That bounds the allocator, not the
stack it was run in; it says nothing about what a larger Nav2 fleet would do.
(vi) \textbf{Arena generality:} All experiments use a single layout, so
cross-environment generalisation is not shown. The arena is fixed in the
placement module; a second layout therefore requires parameterising that
module rather than re-tuning the method. We specified and costed that
experiment but did not run it here.
(vii) \textbf{Single-robot capability:} All robots are identical (ST-SR);
heterogeneous-capability fleets are not evaluated. The cost function already
carries a capability term and the admission gate already rejects incapable
pairs, so the extension is a scenario change rather than a method change. We do
not claim it until it is measured.
...
spawn pose, which deliberately isolates allocation quality from localization
error. Real deployments incur localization drift. The comparison here stays
fair, because the contribution is the allocation policy and every method shares
the identical ground-truth pose; but quantifying robustness to localization
noise (e.g.\ AMCL) is left to future work.
...
$67\pm78$~s over runs spanning 0--263~s). The cause is that some seeds record
no in-window failure recovery at all---6 of the 60 runs at 3 robots and 3 of 80
at 5 robots. The \textit{robot\_failure} scenario guarantees an in-window
failure, so it is the cleaner recovery measure.
...
factor removed moves the priority-weighted proxy fitness by at most $0.8$~pp in
any scenario ($+0.47$~pp robot failure, $p{=}0.046$; $+0.77$~pp mixed stress,
$p{=}0.0015$; $+0.09$~pp deadline pressure, $p{=}0.54$). Every sign is
positive: removing the factor helps, and in two of the three scenarios the
improvement is statistically detectable at 500 paired seeds. The artefact is
therefore small but not invisible in the reported outcomes, and the direction
is the one the audit predicted---the factor is a penalty on urgent work, not
the discount it was written as. We report the implemented form, and leave the
correction to a revision rather than re-tuning the evaluated system after the
fact; a corrected $\rho$ would be expected to improve the numbers in this
paper by under a percentage point, not to change any ranking.
(xiii) \textbf{The geodesic ETA is ablated only on the proxy.} All four
registered closed-loop arms, and the exploratory fifth, ran with the geodesic
oracle enabled, so its contribution is quantified on the navigation proxy alone
(Section~\ref{sec:ablation}). That plane charges nothing for congestion or
recovery latency, so the $+1.8$ to $+4.1$~pp measured there is not a closed-loop
estimate. A matched Gazebo arm with the oracle disabled is the experiment that
would settle it; it is specified but not run here.
...
bounded load-repair step. All three evaluation layers now put AHE-MRTA first in
every scenario, though they disagree on the order below it: commit-once
BiG-MRTA is the consistent runner-up on both simulator planes and finishes last
in two of the three closed-loop scenarios.
On the 500-seed navigation-proxy planes AHE-MRTA leads every scenario,
and eight of nine paired comparisons against a baseline are significant; the
exception is a tie with commit-once BiG-MRTA under robot failure.

At the primary 5-robot Gazebo scale, 20 seeds per cell support corrected
inference: 20 of 63 cross-method comparisons survive Bonferroni correction, 15
in AHE-MRTA's favour. Within the evaluated map and Nav2 stack, AHE-MRTA
reaches full completion under robot failure at 3 and 5 robots and the largest
effective on-time completion under deadline pressure---about $12\%$
(5 robots) and $46\%$ (10 robots) above the strongest baseline, whose
deadline-violation rate it also cuts by $17\%$ and $49\%$---at the cost of
longer routes, lower workload balance, and decision latency above the
commit-once baselines. The 10-robot cells remain descriptive at five seeds.

The matched closed-loop ablation locates the mechanism, and bounds it from
both sides. Pinning the allocator to a paradigm the context does not call for
costs $9$--$11$ percentage points of effective on-time
completion under deadline pressure (corrected $p<0.005$, large effect), while
removing the override cascade changes nothing measurable ($0.650$ versus
$0.641$; raw $p{=}0.59$, $p{=}1.000$ after correction); an exploratory fifth
arm pinned to the paradigm the
fallback almost always returns matches the full selector as well ($0.638$,
$p{=}0.43$). We therefore claim that the portfolio must hold a paradigm suited
to the regime and that the allocator must not be pinned away from it---not the
particular selector rule we implemented, and not its advantage over its own
dominant behaviour. What the
present evidence still does not cover is a second arena---every closed-loop
number here comes from one map---a 10-robot seed budget large enough to
carry correction, and a regime in which spatial-greedy is the wrong answer;
