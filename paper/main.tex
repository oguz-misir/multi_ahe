...
(300 runs). Under deadline pressure AHE-MRTA leads effective on-time
completion ($0.79$ at 5 robots, $0.76$ at 10, against at most $0.66$ and
$0.52$), cutting the strongest baseline's deadline-violation rate by $38\%$
and $49\%$ while deciding in $0.5$--$3.4$~ms. A pre-registered matched
...
portfolio, and explicitly not the particular selector we implemented---a
distinction the navigation proxy, where no variant separates, is structurally
unable to draw.
\end{abstract}
...
(Section~\ref{sec:limitations}). The allocation-only ablation, in which no
variant separates at all, cannot draw this distinction: it does not reproduce
the navigation delays under which a switch pays off.
...
\begin{enumerate}
  ...
  closed-loop Gazebo evidence separately, and shows the three do not agree:
  a pre-registered matched ablation in the closed loop separates the portfolio
  (load-bearing) from the selector rule (not), where the proxy separates
  nothing.
\end{enumerate}
...
the physical stack nor rules one out, because the proxy does not reproduce the
navigation delays, congestion, and recovery latencies under which a switch
would pay off. The matched closed-loop experiment below was run for exactly
that reason, and it does not agree with this table.
