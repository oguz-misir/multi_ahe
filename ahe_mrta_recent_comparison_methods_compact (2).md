...
AHE-MRTA makalesinde karşılaştırma seti şu şekilde güncellenmelidir:

| Grup | Yöntem | Kısa ad | Kullanım rolü |
|---|---|---|---|
| Classical baseline | Greedy Nearest-Target | greedy_nearest | En basit mesafe tabanlı baseline |
| Classical baseline | EDF / Deadline-Aware | deadline_aware | Deadline baskısını izole eder |
| Classical MRTA baseline | Auction-Based MRTA | auction_based | Market-based görev atama ailesini temsil eder |
| Strong fixed baseline | Static Weighted Assignment | static_weighted | Sabit çok kriterli maliyet baseline'ı |
| Recent online graph baseline | BiG-MRTA | big_mrta | Online weighted bipartite graph karşılaştırması |
| Recent adaptive baseline | RoSTAM-EA | rostam_ea | Self-adaptive evolutionary MRTA karşılaştırması |
| Recent communication-efficient baseline | Consensus-DBTA | consensus_dbta | Düşük iletişimli consensus-based MRTA karşılaştırması |
| Proposed method | Full AHE-MRTA | full_ahe_mrta | Önerilen yöntem |
| Ablation | AHE without dominance | ahe_no_dominance | Dominance katkısını test eder |
| Ablation | AHE without cooperation/suppression | ahe_no_cooperation_suppression | Ekolojik etkileşim katkısını test eder |
| Ablation | AHE without event-triggered replanning | ahe_no_event_replanning | Replanning katkısını test eder |

---

# 8. Scenario-to-Method Matching

Tüm yöntemleri tüm senaryolarda çalıştırmak RA-L kısa formatı için gereksiz derecede geniş olabilir. Aşağıdaki plan daha savunulabilir ve adildir.

| Senaryo | Çalıştırılacak yöntemler | Gerekçe |
|---|---|---|
| Dynamic Task Arrival | Greedy, Auction, Static Weighted, BiG-MRTA, Consensus-DBTA, Full AHE | Online görev oluşumu iddiasını test eder. |
| Deadline Pressure | EDF, Static Weighted, BiG-MRTA, Full AHE | Deadline odaklı güçlü ve temiz karşılaştırma sağlar. |
| Robot Failure | Auction, RoSTAM-EA, Consensus-DBTA, Full AHE | Recovery ve replanning iddiasını test eder. |
| High Task Density | Static Weighted, BiG-MRTA, RoSTAM-EA, Full AHE | Ölçek ve workload balance davranışını test eder. |
| Mixed Stress Scenario | Static Weighted, BiG-MRTA, RoSTAM-EA, Consensus-DBTA, Full AHE | AHE'nin asıl farkının görünmesi beklenen ana senaryo. |
| Scalability 5/10/15 robot | Static Weighted, BiG-MRTA, Consensus-DBTA, Full AHE | BiG ve DBTA ölçeklenebilirlik açısından adil rakiplerdir. |
| Ablation | Full AHE + 3 AHE ablation | Yöntemin kendi bileşen katkılarını izole eder. |

---

# 9. Recommended Experiment Matrix

| Deney grubu | Robot/hedef ölçeği | Yöntemler | Tekrar |
|---|---|---|---:|
| Main comparison | 5 robot / 25 hedef | Greedy, EDF, Auction, Static Weighted, BiG-MRTA, Consensus-DBTA, Full AHE | 20 seed |
| Adaptive comparison | 5 robot / 25 hedef | Static Weighted, RoSTAM-EA, Full AHE | 20 seed |
| Stress comparison | 10 robot / 50 hedef | Static Weighted, BiG-MRTA, RoSTAM-EA, Consensus-DBTA, Full AHE | 20 seed |
| Failure/recovery | 5 robot / 25 hedef | Auction, RoSTAM-EA, Consensus-DBTA, Full AHE | 20 seed |
| Ablation | 5 robot / 25 hedef | Full AHE + 3 ablation | 20 seed |
| Scalability | 5/10/15 robot | Static Weighted, BiG-MRTA, Consensus-DBTA, Full AHE | 10-20 seed |
